          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-7 lg:gap-10 items-start">
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-3">
                <div className="rounded-xl bg-[#F6F3ED] border border-[#E8E4DB] p-2.5"><span className="block text-[10px] font-bold uppercase tracking-wider text-[#5B5148]">Texture</span><span className="text-xs font-semibold text-[#304437] line-clamp-1">{product.texture}</span></div>
                <div className="rounded-xl bg-[#EAF1E7] border border-[#DCE7D8] p-2.5"><span className="block text-[10px] font-bold uppercase tracking-wider text-[#405541]">Age guidance</span><span className="text-xs font-semibold text-[#304437] line-clamp-1">{product.ageGuidance}</span></div>
              </div>
            </div>
            ...
            <div className="space-y-6 pr-0 lg:pr-4 bg-transparent p-1 sm:p-2">
              <div className="flex flex-wrap items-center gap-2 pr-12">
                ...
                <span className="flex items-center gap-1 text-xs text-[#3F443F] font-semibold"><Star className="w-3.5 h-3.5 fill-[#C49355] text-[#C49355]"/><b className="text-[#294735]">{product.rating}</b> ({product.reviewCount})</span>
              </div>
              ...
              <div className="border-b border-[#E8E1D7] pb-5">
                ...
                <p className="font-serif-brand italic text-[#405C47] font-semibold mt-2 text-base">“{product.tagline}”</p>
                <p className="text-sm font-medium text-[#332F2B] leading-relaxed mt-4 max-w-xl">{product.shortDescription}</p>
              </div>
              ...
              <div className="rounded-[24px] border border-[#E4DED4] bg-white p-4 sm:p-5 shadow-[0_10px_30px_rgba(53,72,57,0.07)]">
                <div className="flex items-center justify-between mb-3"><div><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[#4A433D]">Choose your routine</p><p className="text-sm font-bold text-[#2E4034] mt-0.5">Purchase options</p></div><span className="text-[11px] font-medium text-[#3D5141] font-semibold">Gentle care, your way</span></div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <button onClick={() => setPurchaseType('subscribe')} className={`relative p-4 rounded-2xl border text-left transition-all ${purchaseType === 'subscribe' ? 'border-[#79A06F] bg-[#E8F3E3] shadow-sm' : 'border-[#D9E5D5] bg-[#F3F8F0] hover:bg-[#EAF4E5]'}`}>
                    <span className="absolute -top-2 right-3 text-[9px] font-bold bg-[#F4B58E] text-[#633B28] border border-[#EAA77E] px-2.5 py-1 rounded-full">SAVE 10%</span><div className="flex items-center justify-between"><p className="text-sm font-bold text-[#274634]">Subscribe & Save</p>{purchaseType === 'subscribe' && <span className="w-5 h-5 rounded-full bg-[#5F8657] text-white flex items-center justify-center"><Check className="w-3 h-3"/></span>}</div><p className="font-serif-brand text-2xl font-bold text-[#274634] mt-3">{formatINR(subscriptionPrice)} <span className="text-[11px] font-sans font-medium">/ delivery</span></p><p className="text-[12px] font-semibold text-[#334A35] mt-1">Flexible repeat deliveries</p>
                  </button>
                </div>

                {purchaseType === 'subscribe' && <div className="mt-3 rounded-2xl bg-[#F4F8F1] border border-[#DCE8D8] p-3.5"><p className="text-xs font-bold text-[#31533C] mb-2">Delivery frequency</p><div className="grid grid-cols-3 gap-2">{([30,60,90] as Frequency[]).map(days => <button key={days} onClick={() => setFrequency(days)} className={`px-2 py-2.5 rounded-xl text-[10px] font-semibold border transition ${frequency === days ? 'bg-[#31533C] border-[#31533C] text-white' : 'bg-white border-[#D6E0D2] text-[#48604A]'}`}>{days} days</button>)}</div><p className="text-[12px] font-semibold text-[#3F4C40] mt-2">Skip, pause or cancel anytime.</p></div>}
